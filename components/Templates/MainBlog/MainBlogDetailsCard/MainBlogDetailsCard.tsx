...
export default function MainBlogDetailsCard({
  addUserFullName,
  ...
  const { mutate: addNewsFavoriteMutate, isPending: addNewsFavoriteIsPending } =useAddNewsFavoriteApi();

  const {mutate: deleteNewsFavoriteMutate,isPending: deleteNewsFavoriteIsPending} = useDeleteNewsFavoriteApi();

  const newsId = id;
  const likeId = currentUserFavoriteId;

  const likeHandler = (event: any) => {
    event.stopPropagation();
    addNewsFavoriteMutate({ NewsId: newsId });
  };

  const dislikeHandler = (event: any) => {
    event.stopPropagation();
        ...
        {!isCurrentUserFavorite ? (
          ...
          <MainTooltip content="حذف از علاقه مندی">
            <Image
              ...
              width={40}
              height={40}
              onClick={(event) => dislikeHandler(event)}
              className="h-[45px] w-[45px]"
            />
          </MainTooltip>
        )}
